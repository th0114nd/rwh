...
rightList = map complement <$> leftOddList
    where complement '0' = '1'
          complement '1' = '0'
          complement _ = error "Need to be a '1' or a '0'!"
...
-- Strict left fold, like foldl1
foldA :: Ix k => (a -> b -> a) -> a -> Array k b -> a
foldA f s ar = go s (indices ar)
            where go s (j:js) = let newS = f s (ar ! j)
                                in newS `seq` go newS js
                  go s _ = s 

foldA1 :: Ix k => (a -> a -> a) -> Array k a -> a
foldA1 f ar = foldA f (ar ! fst (bounds ar)) ar

encodeEAN13 :: String -> String
encodeEAN13 = concat . encodeDigits . map digitToInt

-- This function computes the check digit
...
leftEncode _ = error "Illegal argument to leftEncod"
...
[leftOddSRL, leftEvenSRL, rightSRL, paritySRL] = 
    map asSRL [leftOddList, leftEvenList, rightList, parityList]
...
updateMap :: Parity Digit
          -> Digit              -- New digit
          -> [Parity Digit]      -- Existing key
          -> ParityMap          -- Existing sequence
...
updateFirst key sequ = insertMap key digit (digit:renormalize qes)
